...
class Faculty_Registration:
    def __init__(self, root):
        ...
        #Id
        Id_label=Label(det_frame,text="ID:", font=("times new roman", 12,"bold"),bg="white")
        ...
        #Email
        Email_label=Label(det_frame,text="Email ID:", font=("times new roman", 12,"bold"),bg="white")
        ...
        dep_label.grid(row=3,column=0, padx=10, pady=10, sticky=W)
        ...
        sec_label.grid(row=4, column=0, padx=10, sticky=W)

        sec_Entry=Entry(det_frame, textvariable=self.var_Sec,width=23,font=("times new roman",12,"bold"),bg="white")
        sec_Entry.grid(row=4, column=1, padx=2, pady=10, sticky=W)
        ...
        #SubCode1
        SubCode1_label=Label(det_frame,text="Subject Code 1:", font=("times new roman", 12,"bold"),bg="white")
        SubCode1_label.grid(row=5,column=0, padx=10, pady=10, sticky=W)

        SubCode1_combo=ttk.Combobox(det_frame, values=subcode_li,textvariable=self.var_subCode1,font=("times new roman",12,"bold"), state="readonly")
        SubCode1_combo.current(0)
        SubCode1_combo.grid(row=5, column=1, padx=2, pady=10, sticky=W)

        #SubCode2
        SubCode2_label=Label(det_frame,text="Subject Code 2:", font=("times new roman", 12,"bold"),bg="white")
        SubCode2_label.grid(row=6,column=0, padx=10, pady=10, sticky=W)

        SubCode2_combo=ttk.Combobox(det_frame, values=subcode_li,textvariable=self.var_subCode2,font=("times new roman",12,"bold"), state="readonly")
        ...
        self.faculty_table=ttk.Treeview(table_frame, column=("name", "id", "email", "branch", "section", "subcode1", "subcode2"), yscrollcommand=scroll_y.set)
        ...
        self.faculty_table.heading("branch", text="Branch")
        self.faculty_table.heading("section", text="Section")
    ...
    def get_cursor(self, event=""):
        ...
        self.var_subCode1.set(data[5]),
        self.var_subCode2.set(data[6])
        ...
        if self.var_branch.get()=="Select branch" or self.var_Name.get()=="" or self.var_Roll.get()=="":
            ...
            try:
                ...
                if Update>0:
                    conn = mysql.connector.connect(host='localhost', username='root', password='Chiku@3037', database='attendance-system')
                    my_cursor = conn.cursor() 
                    my_cursor.execute("Update faculty set Name=%s, Email=%s, Branch=%s, Section=%s, SUBCODE_1=%s, SUBCODE_2=%s where Id=%s", (
                                                                                                                    self.var_Name.get(),
                                                                                                                    self.var_Email.get(),
                                                                                                                    self.var_branch.get(),
                                                                                                                    self.var_Sec.get(),
